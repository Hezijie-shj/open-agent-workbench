...
| 模块 | 业务目标 | 已演示能力 |
| --- | --- | --- |
| 银行流水解析 | 对 PDF 流水进行结构化识别、复核和报告生成 | 文件上传、压缩包解压、PDF 拆页、图片渲染、OCR 坐标、模型重试、CSV/JSON 解析、金额校验、敏感词兜底、问题页标记、复核记录、报告下载 |
| 制度文档比对 | 将制度原文与规则库文档进行差异比对 | 文档上传、规则库读取、文本抽取、模型比对、风险分级、文本定位、高亮兜底、状态保存、复核记录、报告输出 |
| 合同文档差异比对 | 对标准合同和多个版本合同进行差异分析 | 多文档上传、格式校验、文本抽取、任务队列、状态轮询、预览链接、本地行级 diff、历史记录、报告输出 |
| 应用市场 | 统一展示可进入的业务流程 | 流程目录、分类筛选、收藏状态、入口跳转 |

## 工程化后端骨架

仓库新增了一个公共基础设施层，位于 `backend/app/platform`，用于模拟正式 SaaS 后端常见的横切能力：

| 文件 | 作用 |
| --- | --- |
| `file_pipeline.py` | 文件上传、压缩包识别、PDF 拆页、页面渲染、空白页/问题页分类 |
| `ocr.py` | OCR 文本块、坐标框、置信度和文本清洗 |
| `model_gateway.py` | 模型调用、提示词名称、重试记录、敏感输出兜底 |
| `parsers.py` | CSV/JSON 解析、字段标准化、金额和余额校验 |
| `repository.py` | 内存数据库、事务回滚、分页筛选、状态机任务、复核记录 |
| `queue.py` | 本地任务队列和异步 worker 行为模拟 |
| `storage.py` | 本地对象存储元数据和校验摘要 |
| `audit.py` | 多租户上下文、审计日志、调用计数和计费占位 |
| `reporting.py` | 报告模板、JSON/DOCX/PDF 下载地址描述 |

这些实现都是公开 demo 版本，不包含真实 OCR、真实模型网关、真实对象存储、真实数据库或内部服务地址。后续要接入真实环境时，可以保持 API 和 agent 边界不变，只替换 `platform` 里的适配器。
...
| 创建上传任务 | `POST /api/v1/bank_statement/projects/upload-task` |
| 工程化全链路 | `POST /api/v1/bank_statement/projects/{project_id}/engineering-pipeline` |
...
| 状态机任务 | `GET /api/v1/bank_statement/workflow-tasks` |
| 复核记录 | `GET /api/v1/bank_statement/review-records` |
| 审计摘要 | `GET /api/v1/bank_statement/audit-summary` |
...
| 创建上传任务 | `POST /api/v1/regulations/tasks/upload-task` |
| 工程化全链路 | `POST /api/v1/regulations/tasks/{task_id}/engineering-pipeline` |
...
| 状态机任务 | `GET /api/v1/regulations/workflow-tasks` |
| 复核记录 | `GET /api/v1/regulations/review-records` |
| 审计摘要 | `GET /api/v1/regulations/audit-summary` |
...
| 创建上传任务 | `POST /api/v1/document_diff/history/upload-task` |
| 创建比对任务 | `POST /api/v1/document_diff/compare-documents` |
| 工程化全链路 | `POST /api/v1/document_diff/engineering-pipeline` |
...
| 状态机任务 | `GET /api/v1/document_diff/workflow-tasks` |
| 复核记录 | `GET /api/v1/document_diff/review-records` |
| 审计摘要 | `GET /api/v1/document_diff/audit-summary` |
...
│  ├─ app/platform      文件、OCR、模型、队列、仓储、审计、报告等公共骨架
│  ├─ app/core          配置加载
